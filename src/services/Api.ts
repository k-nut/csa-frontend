...
class Api {
  private async request<T>(
    ...
    const response = await this.requestUnChecked(endpoint, options);
  ...
  private async requestUnChecked<T>(
    endpoint: string,
    options: RequestInit = {}
  ): Promise<Response> {
  ...
  changePassword = async (
    ...
    | { ok: true; error: null }
    | { ok: false; error: { type: "min_length"; value: number } }
  > => {
    const userId = authState.getId();
    const response = await this.requestUnChecked(`/users/${userId}`, {
      method: "PATCH",
      body: JSON.stringify({ password }),
    });

    if (response.status === 400) {
      const error = (await response.json())?.validation_error?.body_params?.[0];
      if (error.type === "string_too_short") {
        return {
          ok: false,
          error: { type: "min_length", value: error.ctx.min_length },
        } as const;
      } else {
